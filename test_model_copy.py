...
fan_data_path = "data/vel/s3/fan/189249773/189249773_20230101_011247_vel_freq.csv"
...
vertical_variance = np.abs(fan_belt_df[:, 0:1] - np.max(fan_belt_df[:, 0:1]))
horizontal_variance = np.abs(fan_belt_df[:, 1:2] - np.max(fan_belt_df[:, 1:2]))
axial_variance = np.abs(fan_belt_df[:, 2:] - np.max(fan_belt_df[:, 2:]))
fan_belt_input_data = np.concatenate([fan_belt_df, fan_belt_squared_values, vertical_variance, horizontal_variance, axial_variance], axis=-1)
...
fan_flow_input_data = np.concatenate([fan_flow_df_1, fan_flow_df_2, fan_flow_df_3])
fan_flow_actual_data = np.reshape(fan_flow_input_data, (1,128,2,1))
# Fan bearing
fan_bearing_df = fan_data.to_numpy()[]
# Model Prediction
